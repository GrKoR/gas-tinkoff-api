 ...
 * @param {any}    dummy  Случайное число или дата. В расчете не используется. Нужно для принудительного пересчета формулы.
 ...
 * @customfunction
 */
function getBidAskSpreadByTicker(ticker, dummy) {
  // dummy parameter is optional
  const figi = _getFigiByTicker(ticker);
  const { tradeStatus, bids, asks } = tinkoffClient.getOrderbookByFigi(figi, 1);
  if (tradeStatus != 'NotAvailableForTrading') return (asks[0].price - bids[0].price) / asks[0].price;
  else return null;
}
...
function getTradesByFigi(figi, from, to) {
  ...
  const values = [['ID', 'Date', 'Operation', 'Ticker', 'Quantity', 'Price', 'Currency', 'SUM', 'Commission']];
  for (let i = operations.length - 1; i >= 0; i--) {
    const { operationType, status, trades, id, date, currency, commission } = operations[i];
    if (operationType == 'BrokerCommission' || status == 'Decline' || operationType == 'Dividend') continue;
    let [totalQuantity, totalSum, weigthedPrice] = _calculateTrades(trades); // calculate weighted values
    if (operationType == 'Buy') {
      // inverse values in a way, that it will be easier to work with
...
function getPortfolio() {
  ...
  for (let i = 0; i < portfolio.length; i++) {
    let { ticker, name, instrumentType, balance, averagePositionPrice, averagePositionPriceNoNkd, expectedYield } =
      portfolio[i];
    let NKD = null;
    ...
    values.push([
      ...
      averagePositionPrice.value * balance,
      expectedYield.value,
      averagePositionPrice.value * balance + expectedYield.value,
      NKD,
...
class _TinkoffClientV2 {
  ...
  _makeApiCall(methodUrl, data) {
    ...
    do {
      ...
      if (rateLimited) {
        // Выжидаем конец периода квоты запросов
    ...
    if (respCode == 200) return respObj;
    else throw new Error(`Ошибка ${respCode} - ${respObj.message} - ${respObj.description}`);
  }
...
function TI_GetInstrumentsID() {
  const AllInstruments = tinkoffClientV2
    ._Bonds('INSTRUMENT_STATUS_BASE')
    .instruments.concat(
...
function TI_GetBidAskSpread(ticker) {
  ...
  if (figi) {
    ...
    if (bids.length > 0 && asks.length > 0)
      return (
        (Number(asks[0].price.units) +
          asks[0].price.nano / 1000000000 -
          (Number(bids[0].price.units) + bids[0].price.nano / 1000000000)) /
        (Number(asks[0].price.units) + asks[0].price.nano / 1000000000)
      );
    else return null;
  }
}
...
function TI_GetPortfolio(accountId) {
  ...
  for (let i = 0; i < portfolio.positions.length; i++) {
    ...
    let averagePositionPrice =
      Number(portfolio.positions[i].averagePositionPrice.units) +
      portfolio.positions[i].averagePositionPrice.nano / 1000000000;
    let currentNkd = null;
...
function TI_GetOperations(accountId, from_param, to_param) {
  ...
  do {
    ...
    const data = tinkoffClientV2._GetOperationsByCursor(
      accountId,
      null,
      from,
      to,
